...
- `meta.traceId` for support and correlation

For most apps, send `authCode` and `codeVerifier` to your backend immediately over HTTPS and perform the exchange there. Keep OAuth client credentials and token verification on the server, not in the iOS app.
...
If your app truly needs an SDK-managed exchange path, `HawcxV1OAuthExchangeService` is available as an advanced option. When configured with a verifier or a non-empty `publicKeyPem`, it can verify the returned ID token and returns `HawcxV1OAuthToken` (`idToken`, `tokenType`, `expiresIn`) instead of app-specific access / refresh tokens.

For backend implementation examples, see:
